...
//	*sbi.bots[c_num].name = m_a;
	...
	//Info_SetValueForKey(cl->userinfo,"skin",cl->name,MAX_INFO_STRING);

	strncpy (cl->pskin, TF_GetSkin(pc_num), sizeof(cl->pskin)-1);
	cl->pskin[sizeof(cl->pskin) - 1] = 0;
	Info_SetValueForKey(cl->userinfo,"skin",cl->pskin,MAX_INFO_STRING);

	strncpy (cl->bname, m_a, sizeof(cl->bname)-1);
	cl->bname[sizeof(cl->bname) - 1] = 0;
	Info_SetValueForKey(cl->userinfo,"bname",cl->bname,MAX_INFO_STRING);
...
char *PF_VarString2 (progfuncs_t *prinst, int	first, globalvars_t2 *pr_globals)
{
	int		i;
