...
//			self->tfstate = ( self->tfstate - ( self->tfstate & 2 ) );
	...
	{
		self->tfstate = self->tfstate | TFSTATE_RELOADING;
		self->s.v.oldweaponmodel = self->s.v.weaponmodel;
		self->s.v.oldcurrentammo = self->s.v.currentammo;
...
//	vec_b = trace_endpos - ( v_forward * 8 );
	if ( g_globalvars.trace_fraction != 1 )
	{
		...
		if ( self->s.v.oldweaponmodel != "" )
		{
			self->tfstate = ( self->tfstate - ( self->tfstate & TFSTATE_RELOADING ) );
		}
		self->s.v.weaponmodel = self->s.v.oldweaponmodel;
		self->s.v.currentammo = self->s.v.oldcurrentammo;
		Attack_Finished ( 0.7 );
		self->s.v.impulse = 0;
		self->StatusRefreshTime = g_globalvars.time + 0.1;
	}
