...
void idClipModel::LoadModel( const idTraceModel &trm, const idMaterial *material, bool notHashed ) {
	if ( !notHashed || traceModelIndex == -1 ) {			// 3j_todo: properly checking if hashed?
		FreeModel();
...
// ddynerman: SD's clip sector code
	int coords[ 4 ];
	gameLocal.clip.CoordsForBounds( coords, absBounds );

	int x, y;
	for( x = coords[ 0 ]; x < coords[ 2 ]; x++ ) {
		for( y = coords[ 1 ]; y < coords[ 3 ]; y++ ) {
			clipSector_t* sector = &gameLocal.clip.clipSectors[ x + ( y << CLIPSECTOR_DEPTH ) ];
...
bool idClip::Translation( trace_t &results, const idVec3 &start, const idVec3 &end,
	...
	// 3j: so material will actually be null...
	memset( &(results.c), 0, sizeof( results.c ) );
	memset( &(trace.c), 0, sizeof( trace.c ) );
