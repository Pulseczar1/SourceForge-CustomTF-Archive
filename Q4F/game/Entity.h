...
// abahr:
	...
	ENTDEF					GetEntityDef( void ) { return static_cast<ENTDEF>(declManager->DeclByIndex( DECL_ENTITYDEF, entityDefNumber, false ) ); };
...
// abahr: added virtual to UpdateModelTransform
	...
	bool					StartSoundShader( const idSoundShader *shader, const s_channelType channel, int soundShaderFlags, bool broadcast, int *length, bool force = false );
...
// scork: accessors so sound editor can indicate current-highlit ent
	...
	virtual idVec3			GetCenter( void )  const { return GetPhysics()->GetAbsBounds().GetCenter(); };
...
// bdube: made virtual
	...
	virtual void			InitDefaultPhysics( const idVec3 &origin, const idMat3 &axis );
...
public:
	...
	virtual void			WaterLevelChange( float oldLevel, float newLevel ) {};

	virtual idVec3&			GetTeamColor( bool visibleColor = false );
	virtual void			UpdateVisibleColor( void );

	virtual void			EmpTouch( const AttackerInfo &attacker, idVec3 &dir, float damageScale );				// called when an emp goes off near this entity

	virtual void			Damage_TF( const TFDamage &dmg, float damageScale );
	virtual void			DamageFeedback_TF( idEntity *victim, const TFDamage &tfDamage );
	virtual void			Killed_TF( const TFDamage &tfDamage, int healthLoss );

	virtual void			Push( idVec3 origin, float amount ) {};

	virtual bool			Give_TF( const char* type, int amount, bool allowLower = true, bool exceedMax = false, bool setAbsolute = false, int* invAmount = NULL, int* invMaxAmount = NULL );
	bool					AdjustIntStat( int &stat, int amount, int min, int max, int maxNet, bool exceedMax, bool allowLower, bool setAbsolute, int* invAmount = NULL, int* invMaxAmount = NULL );

	TFTeam*					GetTeam( void ) { return myTeam; };

	virtual bool			IsHolding( TFCarryItem* item ) { return false; };		// inherited by players

	ENTDEF					GetSpawnArgsDef( const char* name );
	const idDict*			GetSpawnArgsDict( const char* name );

	int						latchedTeam;			// need to track when team gets changed
	int						team;					// what team this entity belongs to
...
protected:
	...
	const idSoundShader*			GetSoundShader( const char* snd_name );

	// events
	void					Event_GetInvAmount				( const char* type );
	void					Event_GetMaxInvAmount			( const char* type );
	void					Event_GiveInvTF					( const char* type, float amount, bool exceedMax );
	void					Event_SetInvTF					( const char* type, float amount, bool allowLower );

	void					Event_PlayWorldSound			( const char* snd_name, int channel );
	void					Event_PlayTeamSound				( int teamNum, const char* snd_name, idEntity* ignore );
	void					Event_PlayNonTeamSound			( int nonTeamNum, const char* snd_name, idEntity* ignore );
	void					Event_PlaySoundTo				( const char* snd_name, idEntity* to );


	void					Event_IsInGroup					( const char* group );	
	void					Event_JoinGroups				( const char* newGroups );
	void					Event_LeaveGroups				( const char* newGroups );

	void					Event_GetTeamNum				( void );
	void					Event_GetTeamName				( void );

	void					Event_PrintMessage				( const char* msg );
	void					Event_PrintTeamMessage			( const char* msg );
	void					Event_PrintNonTeamMessage		( const char* msg );

	void					Event_PlayEffect				( const char* effectName, idVec3 originOffset, idVec3 endOriginOffset, idVec3 dirOffset, bool loop, bool align );
	void					Event_StopEffect				( const char* effectName );
	void					Event_StopAllEffects			( void );

	void					Event_GetSpawnPos				( void);

	void					Event_IsPlayer					( void );

	void					Event_IsHolding					( const char* entName );

	TFTeam*					myTeam;

	float					teamColorScale;
...
ID_INLINE ENTDEF idEntity::GetSpawnArgsDef( const char* name ) {
	const char* defStr = NULL;
	if ( spawnArgs.GetString( name, "", &defStr ) && defStr && defStr[0] ) {
		return gameLocal.FindEntityDef( defStr, false );
	}

	return NULL;
}

ID_INLINE const idDict* idEntity::GetSpawnArgsDict( const char* name ) {
	ENTDEF def = GetSpawnArgsDef( name );
	if ( def ) {
		return &(def->dict);
	}
...
ID_INLINE idVec3& idEntity::GetTeamColor( bool visibleColor ) {
	return tfGame.GetTeamColor( team );
}

ID_INLINE void idEntity::UpdateVisibleColor( void ) {
	if ( fl.teamColored ) {
