...
void idMover::Event_TeamBlocked( idEntity *blockedEntity, idEntity *blockingEntity ) {
	if ( !blockingEntity->fl.takedamage ) {
		if ( blockingEntity->IsType( idMoveable::GetClassType() )
...
void idDoor::Spawn( void ) {
	...
	AttackerInfo att;
	att.Create( this );
	tfDamage.Create( att, "damage_moverCrush" );
	tfDamage.SetDamage( spawnArgs.GetInt( "damage", "0" ) );
...
void idDoor::Event_PartBlocked( idEntity *blockingEntity ) {
	if ( tfDamage.GetDamage() > 0 ) {
		//blockingEntity->Damage( this, this, vec3_origin, "damage_moverCrush", damage, INVALID_JOINT );
		blockingEntity->Damage_TF( tfDamage, 1.0f );
	}
}
...
// jnewquist: Use accessor for static class type 
	...
	// mirror player across us
	idVec3 normal = trace->c.normal;
	normal *= - (

	p->SetOrigin( center + dir );
