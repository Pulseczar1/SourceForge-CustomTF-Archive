...
==================
*/
static void Cmd_Say( chatTargetType_t target, const idCmdArgs &_args ) {

	idStr text;
	const char *cmd = _args.Argv( 0 );
	...
	if ( _args.Argc() < 2 ) {
		gameLocal.Printf( "usage: %s <&sound> <text>\n", cmd );
		return;
	}

	int shaderIndex = 0;
	tfGame.ExtractChatMessage( _args.Args(), text, shaderIndex );
	...
	if ( gameLocal.isClient ) {
		...
		gameLocal.mpGame.ProcessChatMessage( gameLocal.localClientNum, target, text, shaderIndex );
	}
}
...
static void Cmd_RecordViewNotes_f( const idCmdArgs &args ) {
	...
	player->GetViewPos( origin, axis, weaponAxis );
...
*/
	...
	// jshepard
	//cmdSystem->AddCommand( "forceTeamChange",				idMultiplayerGame::ForceTeamChange_f,			CMD_FL_GAME,		"force team change: forceTeamChange <id> <teamname>" );
