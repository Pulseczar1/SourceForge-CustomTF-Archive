...
void idFreeView::SetFreeView( int clientNum ) {
	...
	gameLocal.clip.Translation( trace, start, end, physics->GetClipModel(), mat3_identity, MASK_PLAYERSOLID, NULL, NULL );
...
===============
*/
void idFreeView::PickRandomSpawn( void ) {			// 3j_todo
	if ( !physics ) {
		Setup();
	}
...
void idFreeView::Setup( void ) {
	...
	physics->SetSpeed( pm_spectatespeed.GetFloat(), pm_spectatespeed.GetFloat() );
