...
;1041:#define UNSIGNED	0x00000200		/* unsigned integer. */
;1042:
;1043:#define to_digit(c)		((c) - '0')
;1044:#define is_digit(c)		((unsigned)to_digit(c) <= 9)
;1045:#define to_char(n)		((n) + '0')
;1046:
;1047:#undef TRY_APPEND
;1048:#define TRY_APPEND(ch) do { if (fill < maxlen) { *buf++ = (ch); } fill++; } while (0)
;1049:
;1050:int AddInt( char **buf_p, size_t maxlen, int val, int width, int flags, char *charset ) {
line 1058
;1051:	char	text[32];
;1052:	int		digits;
;1053:	int		signedVal;
...
NEU4 $256
line 1060
;1060:		charset = "0123456789";  /* Decimal by default. */
ADDRFP4 20
ADDRGP4 $258
ASGNP4
LABELV $256
line 1062
;1061:
;1062:	for (base = 0; charset[base]; base++);  /* Note NOP operation. */
ADDRLP4 44
CNSTI4 0
ASGNI4
ADDRGP4 $262
...
NEI4 $259
line 1065
;1063:	/* Offset of NULL is the number base.  So "0123456789" -> base-10. */
;1064:
;1065:	digits = 0;
ADDRLP4 8
CNSTI4 0
...
;1070:	do {
line 1071
;1071:		text[digits++] = charset[val % base]; //'0' + val % 10;
ADDRLP4 52
ADDRLP4 8
INDIRI4
ASGNI4
...
line 1075
;1074:
;1075:	if (( signedVal < 0 ) && !(flags & UNSIGNED)) {
ADDRLP4 52
CNSTI4 0
ASGNI4
ADDRLP4 48
...
;1084:			TRY_APPEND(( flags & ZEROPAD ) ? '0' : ' ');
ADDRLP4 0
INDIRI4
ADDRFP4 4
INDIRI4
...
LABELV $286
line 1091
;1090://		*buf++ = text[digits];
;1091:		TRY_APPEND(text[digits]);
ADDRLP4 0
INDIRI4
ADDRFP4 4
INDIRI4
...
LABELV $296
line 1098
;1097://			*buf++ = ( flags & ZEROPAD ) ? '0' : ' ';
;1098:			TRY_APPEND(( flags & ZEROPAD ) ? '0' : ' ');
ADDRLP4 0
INDIRI4
ADDRFP4 4
INDIRI4
...
;1104:}
;1105:
;1106:int AddFloat( char **buf_p, size_t maxlen, float fval, int width, int prec ) {
line 1115
;1107:	char	text[32];
;1108:	int		digits;
;1109:	float	signedVal;
;1110:	char	*buf;
;1111:	int		val;
;1112:	int		zerofrac;  /* Fractional part remains zero? */
;1113:	int		fill;
;1114:
;1115:	fill = 0;
ADDRLP4 0
...
LABELV $323
line 1144
;1143://		*buf++ = text[digits];
;1144:		TRY_APPEND(text[digits]);
ADDRLP4 0
INDIRI4
ADDRFP4 4
INDIRI4
...
LABELV $328
line 1152
;1151:	// write the fraction
;1152:	zerofrac = 1; /* Assume fractional part is zero. */
ADDRLP4 48
CNSTI4 1
ASGNI4
line 1153
...
ASGNI1
line 1159
;1159:		zerofrac &= (val == 0); /* Does fractional part remain zero? */
ADDRLP4 44
INDIRI4
CNSTI4 0
NEI4 $334
...
EQI4 $336
line 1165
;1163:		/* Fractional part is zero.  Don't bother with fractional. */
;1164:		/* Um... how do we figure out if we need to do alternate form? */
;1165:		return fill;
ADDRLP4 0
INDIRI4
RETI4
...
LABELV $341
line 1172
;1172:		for (prec = 0; prec < digits; prec++) {
ADDRFP4 16
CNSTI4 0
ASGNI4
ADDRGP4 $348
...
;1180:
;1181:
;1182:int AddString( char **buf_p, size_t maxlen, char *string, int width, int prec ) {
line 1187
;1183:	int		size;
;1184:	char	*buf;
;1185:	int		fill;
...
LTI4 $358
line 1196
;1196:		for( size = 0; size < prec; size++ ) {
ADDRLP4 8
CNSTI4 0
ASGNI4
ADDRGP4 $363
...
;1225:vsprintf
;1226:
;1227:I'm not going to support a bunch of the more arcane stuff in here
;1228:just to keep it simpler.  For example, the '*' and '$' are not
;1229:currently supported.  I've tried to make it so that it will just
;1230:parse and ignore formats we don't support.
;1231:*/
;1232:int vsprintf( char *buffer, const char *fmt, va_list argptr ) {
;1233:	int		*arg;
;1234:	char	*buf_p;
;1235:	char	ch;
;1236:	int		flags;
...
;1244:
;1245:	while( true ) {
;1246:		// run through the format string until we hit a '%' or '\0'
;1247:		for ( ch = *fmt; (ch = *fmt) != '\0' && ch != '%'; fmt++ ) {
;1248:			*buf_p++ = ch;
;1249:		}
;1250:		if ( ch == '\0' ) {
;1251:			goto done;
...
;1304:		case 'd':
;1305:		case 'i':
;1306:			AddInt( &buf_p, *arg, width, flags, "0123456789" );
;1307:			arg++;
;1308:			break;
;1309:		case 'f':
;1310:		case 'g':
;1311:			AddFloat( &buf_p, *(double *)arg, width, prec );
;1312:#ifdef __LCC__
;1313:			arg += 1;	// everything is 32 bit in my compiler
;1314:#else
;1315:			arg += 2;
;1316:#endif
;1317:			break;
;1318:		case 's':
;1319:			AddString( &buf_p, (char *)*arg, width, prec );
;1320:			arg++;
;1321:			break;
;1322:		case '%':
;1323:			*buf_p++ = ch;
;1324:			break;
;1325:
;1326:		case 'u':  /* Unsigned decimal. */
;1327:			AddInt (&buf_p, *arg, width, flags | UNSIGNED, "0123456789");
;1328:			arg++;
;1329:			break;
;1330:		case 'o':  /* (Unsigned) Octal. */
;1331:			AddInt (&buf_p, *arg, width, flags | UNSIGNED, "01234567");
;1332:			arg++;
;1333:			break;
;1334:		case 'x':  /* (unsigned) hexadecimal (lower-case letters). */
;1335:			AddInt (&buf_p, *arg, width, flags | UNSIGNED, "0123456789abcdef");
;1336:			arg++;
;1337:			break;
;1338:		case 'X':  /* (unsigned) Hexadecimal (upper-case letters). */
;1339:			AddInt (&buf_p, *arg, width, flags | UNSIGNED, "0123456789ABCDEF");
;1340:			arg++;
;1341:			break;
;1342:
;1343:		default:
...
;1350:done:
;1351:	*buf_p = 0;
;1352:	return buf_p - buffer;
;1353:}
;1354:#endif /* 0 */
;1355:
;1356:/* this is really crappy */
;1357:int sscanf( const char *buffer, const char *fmt, ... ) {
line 1362
;1358:	int		cmd;
;1359:	int		**arg;
;1360:	int		count;
...
;1391:
;1392:
;1393:/* vsnprintf, extended version of Q3A 1.29h bg_libc.c:vsprintf() */
;1394:/* Return value is how many characters filled, or how many would have been filled had there been enough space. */
;1395:/*
;1396:vsnprintf
;1397:
;1398:I'm not going to support a bunch of the more arcane stuff in here
;1399:just to keep it simpler.  For example, the '*' and '$' are not
;1400:currently supported.  I've tried to make it so that it will just
;1401:parse and ignore formats we don't support.
;1402:*/
;1403:int _vsnprintf( char *buffer, size_t size, const char *fmt, va_list argptr ) {
line 1417
;1404:	int		*arg;
;1405:	char	*buf_p;
;1406:	char	ch;
...
;1410:	int		n;
;1411:	char	sign;
;1412:	int		pneumonoultramicroscopicsilicovolcanoconiosis; /* Just for you, vogon_jeltz. */
;1413:
;1414:#undef TRY_APPEND
;1415:#define TRY_APPEND(ch) do { if (pneumonoultramicroscopicsilicovolcanoconiosis < size) { *buf_p++ = (ch); } pneumonoultramicroscopicsilicovolcanoconiosis++; } while (0)
;1416:
;1417:	if (size < 1)
ADDRFP4 4
INDIRI4
...
LABELV $395
line 1419
;1419:	pneumonoultramicroscopicsilicovolcanoconiosis = 0;
ADDRLP4 0
CNSTI4 0
ASGNI4
line 1420
;1420:	size--; /* Make room for trailing \0. */
ADDRFP4 4
ADDRFP4 4
INDIRI4
CNSTI4 1
...
;1424:	while( true ) {
line 1426
;1425:		// run through the format string until we hit a '%' or '\0'
;1426:		for ( ch = *fmt; (ch = *fmt) != '\0' && ch != '%'; fmt++ ) {
ADDRLP4 4
ADDRFP4 8
INDIRP4
INDIRI1
...
;1485:		case 'd':
;1486:		case 'i':
;1487:			pneumonoultramicroscopicsilicovolcanoconiosis += AddInt( &buf_p, size - pneumonoultramicroscopicsilicovolcanoconiosis, *arg, width, flags, "0123456789" );
ADDRLP4 8
ARGP4
ADDRFP4 4
INDIRI4
...
;1490:		case 'f':
;1491:		case 'g':
;1492:			pneumonoultramicroscopicsilicovolcanoconiosis += AddFloat( &buf_p, size - pneumonoultramicroscopicsilicovolcanoconiosis, *(double *)arg, width, prec );
ADDRLP4 8
ARGP4
ADDRFP4 4
INDIRI4
...
line 1494
;1493:#ifdef __LCC__
;1494:			arg += 1;	// everything is 32 bit in my compiler
ADDRLP4 16
ADDRLP4 16
INDIRP4
CNSTI4 4
...
line 1500
;1499:		case 's':
;1500:			pneumonoultramicroscopicsilicovolcanoconiosis += AddString( &buf_p, size - pneumonoultramicroscopicsilicovolcanoconiosis, (char *)*arg, width, prec );
ADDRLP4 8
ARGP4
ADDRFP4 4
INDIRI4
...
line 1509
;1507:
;1508:		case 'u':  /* Unsigned decimal. */
;1509:			pneumonoultramicroscopicsilicovolcanoconiosis += AddInt (&buf_p, size - pneumonoultramicroscopicsilicovolcanoconiosis, *arg, width, flags | UNSIGNED, "0123456789");
ADDRLP4 8
ARGP4
ADDRFP4 4
INDIRI4
...
LABELV $448
line 1513
;1512:		case 'o':  /* (Unsigned) Octal. */
;1513:			pneumonoultramicroscopicsilicovolcanoconiosis += AddInt (&buf_p, size - pneumonoultramicroscopicsilicovolcanoconiosis, *arg, width, flags | UNSIGNED, "01234567");
ADDRLP4 8
ARGP4
ADDRFP4 4
INDIRI4
...
LABELV $450
line 1517
;1516:		case 'x':  /* (unsigned) hexadecimal (lower-case letters). */
;1517:			pneumonoultramicroscopicsilicovolcanoconiosis += AddInt (&buf_p, size - pneumonoultramicroscopicsilicovolcanoconiosis, *arg, width, flags | UNSIGNED, "0123456789abcdef");
ADDRLP4 8
ARGP4
ADDRFP4 4
INDIRI4
...
LABELV $452
line 1521
;1520:		case 'X':  /* (unsigned) Hexadecimal (upper-case letters). */
;1521:			pneumonoultramicroscopicsilicovolcanoconiosis += AddInt (&buf_p, size - pneumonoultramicroscopicsilicovolcanoconiosis, *arg, width, flags | UNSIGNED, "0123456789ABCDEF");
ADDRLP4 8
ARGP4
ADDRFP4 4
INDIRI4
...
ASGNI1
line 1536
;1535://	return buf_p - buffer;
;1536:	return pneumonoultramicroscopicsilicovolcanoconiosis;
ADDRLP4 0
INDIRI4
RETI4
LABELV $394
endproc _vsnprintf 136 24
export vsprintf
proc vsprintf 4 16
line 1540
;1537:}
;1538:
;1539:
;1540:int vsprintf( char *buffer, const char *fmt, va_list argptr ) {
line 1541
;1541:  return _vsnprintf(buffer, INT_MAX, fmt, argptr);
ADDRFP4 0
INDIRP4
ARGP4
CNSTI4 2147483647
...
RETI4
LABELV $466
endproc vsprintf 4 16
export _snprintf
proc _snprintf 12 16
line 1545
;1542:}
;1543:
;1544:int _snprintf(char *buffer, int size, const char *fmt, ...)
;1545:{
line 1549
;1546:  va_list vp;
;1547:  int retval;
;1548:
;1549:  va_start(vp, fmt);
ADDRLP4 0
ADDRFP4 8+4
ASGNP4
line 1550
;1550:  retval = _vsnprintf(buffer, size, fmt, vp);
ADDRFP4 0
INDIRP4
ARGP4
ADDRFP4 4
...
RETI4
LABELV $467
endproc _snprintf 12 16
export asin
proc asin 8 8
line 1556
;1553:}
;1554:
;1555:float asin(float x)
;1556:{
line 1557
;1557:        return atan2(x, -x*x+1);
ADDRLP4 0
ADDRFP4 0
INDIRF4
ASGNF4
...
;1558:}
;1559:
;1560:int memcmp (const void *s1, const void *s2, size_t n)
;1561:{
line 1562
;1562:	register unsigned const char *p1 = s1, *p2 = s2;
ADDRLP4 0
ADDRFP4 0
INDIRP4
ASGNP4
